...
public void batal() {
    ...
    private void initComponents() {
        ...
        btnbahasa.setText("Form Data Matkul");
    ...
    // Jika tidak ada baris yang dipilih, tampilkan peringatan dan keluar dari method
    if (selectedRow == -1) {
    ...
    try {
        // Mengambil input dari komponen form
        String kode = txtKode.getText();
        String mapel = txtNama.getText();
        String guru = txtGuru.getText();
        String kategori = (String) cmb_pilih.getSelectedItem();
        int semester = Integer.parseInt(txtSemester.getText()); // Parsing semester jadi integer

        // Mengambil objek ManajemenMapel dari listData berdasarkan baris yang dipilih
        ManajemenMapel mp = listData.get(selectedRow);

        // Memperbarui data dalam objek dengan data yang baru dari form
        mp.dataKode(kode);
        mp.dataNama(mapel);
        mp.dataGuru(guru);
        mp.dataKategori(kategori);
        mp.dataSemester(semester);

        // Memperbarui data di tabel (model tabel) agar tampilannya ikut berubah
        DefaultTableModel model = (DefaultTableModel) tabel_data.getModel();
        model.setValueAt(kode, selectedRow, 0);
        model.setValueAt(mapel, selectedRow, 1);
        model.setValueAt(guru, selectedRow, 2);
        model.setValueAt(kategori, selectedRow, 3);
        model.setValueAt(semester, selectedRow, 4);

        // Menampilkan pesan sukses
        JOptionPane.showMessageDialog(this, "Data berhasil diubah!");

        // Mengosongkan field input setelah proses update
        batal();
        ...
        // Jika data tidak ditemukan di listData, tampilkan error
        JOptionPane.showMessageDialog(this, "Data tidak ditemukan di listData!", "Error", JOptionPane.ERROR_MESSAGE);
    }
    ...
    // Membuat objek baru dari class ManajemenMapel
    ManajemenMapel mhs = new ManajemenMapel();
    // Mengisi data pada objek mhs berdasarkan inputan dari textfield
    mhs.dataKode(txtKode.getText()); // Mengisi kode mata pelajaran
    mhs.dataNama(txtNama.getText()); // Mengisi nama mata pelajaran
    mhs.dataGuru(txtGuru.getText()); // Mengisi nama guru
    ...
    if (kategoriTerpilih != null) {
        // Jika kategori dipilih, simpan datanya ke dalam objek
        mhs.dataKategori(kategoriTerpilih);
    } else {
    ...
    try {
        ...
        mhs.dataSemester(semester);

        // Menambahkan data ke list untuk dimasukkan ke tabel
        list.add(mhs.cetakKode());
        list.add(mhs.cetakNama());
        list.add(mhs.cetakGuru());
        list.add(mhs.cetakKategori());
        list.add(mhs.cetakSemester());

        // Menambahkan list sebagai baris baru ke tabel
        dataModel.addRow(list.toArray());

        // Menambahkan objek ke list data utama
        listData.add(mhs);

        // Mereset/mengosongkan inputan setelah data ditambahkan
        batal();

    } catch (NumberFormatException e) {
        // Jika input semester bukan angka, tampilkan pesan error
        JOptionPane.showMessageDialog(this, "Semester harus berupa angka!", "Error", JOptionPane.ERROR_MESSAGE);
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GUI_Mapel.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
